...
module problem_setup
  ...
  integer,  parameter :: Nx = 20        ! Number of gridpoints in x
  integer,  parameter :: Ny = 20       ! Number of gridpoints in y
...
module matrices
  ...
  use type_defs
  implicit none
  real(dp), allocatable, dimension(:,:) :: LapUV,LapP,LapPBig,LapPBigD,EU,EC,MI,FI,pbvecbig_M
  integer,  allocatable, dimension(:) :: ipiv_pbig,ipiv_uv
  real(dp), allocatable, dimension(:) :: pvec,lpvec,uvec,vvec,uvec_k,vvec_k,uvec_temp,vvec_temp,EU_temp,MI_temp,FI_temp
...
program ins
  ...
  integer :: i,j,ik,sys_size_p,sys_size_pbig,info,nt,sys_size_uv,iter
  character(100) :: str
...
,MI_temp((Nx+1)*(Ny+1)),FI_temp((Nx+1)*(Ny+1)),EU_temp((Nx+1)*(Ny+1)))
...
,uvec_temp((Nx-1)*(Ny-1)),vvec_temp((Nx-1)*(Ny-1)))
  ...
  if (use_dense_p) then
    ...
    ! call printdble2d(LapP,1,(nx+1)*(ny+1),1,(nx+1)*(ny+1),'LapP.txt')
    allocate(LapPBig((Nx+1)*(Ny+1)+1,(Nx+1)*(Ny+1)+1))
    allocate(LapPBigD((Nx+1)*(Ny+1)+1,(Nx+1)*(Ny+1)+1))
    allocate(EC(3,3))
    allocate(EU((Nx+1)*(Ny+1),3))
    allocate(MI((Nx+1)*(Ny+1),3))
    allocate(FI((Nx+1)*(Ny+1),1))
    allocate(pbvecbig_M((Nx+1)*(Ny+1)+1,1))
    LapPBig = 1.0_dp
    LapPBig((Nx+1)*(Ny+1)+1,(Nx+1)*(Ny+1)+1) = 0.0_dp
    LapPBig(1:(Nx+1)*(Ny+1),1:(Nx+1)*(Ny+1)) = LapP
    ! Set up dense linear algebra stuff.
    ...
    do i = 1,sys_size_pbig-1
        do j = 1,sys_size_pbig-1
            LapPbigD(i,j) = LapPbig(i,j)
        end do
    end do
    LapPbigD(1,1) = 2*LapPbig(1,1)
    LapPbigD(sys_size_pbig-1,sys_size_pbig-1) = 2*LapPbig(sys_size_pbig-1,sys_size_pbig-1)
    EU(1,1) = 1
    do i = 2,sys_size_pbig-1
        EU(i,1) = 0
    end do
    EU(sys_size_pbig-1,2) = 1
    do i = 1,sys_size_pbig-2
        EU(i,2) = 0
    end do
    do i = 1,sys_size_pbig-1
        EU(i,3) = 1
        ...
        do while (( res/norm .gt. 1e-4) &
               .and. (iter .lt. 1e12))
                ...
                do i = 1,sys_size_pbig-1
                    r = EU(i,ik)
                    do j = 1,sys_size_pbig-1
                        r = r - LapPbigD(i,j)*MI(j,ik)
                    end do
                    res = res+r**2
                    MI(i,ik) = MI(i,ik) + r/LapPbigD(i,i)
                end do
            iter = iter + 1
        end do
    end do
    
    MI = matmul(MI,inv(EC+matmul(transpose(EU),MI)))
        FI_temp = FI(:,1)+1
        iter = 0
        norm = sum(pbvecbig**2)
        res = norm
        do while (( res/norm .gt. 1e-4) &
               .and. (iter .lt. 1e12))
                ...
                do i = 1,sys_size_pbig-1
                    r = pbvecbig(i)
                    do j = 1,sys_size_pbig-1
                        r = r - LapPbigD(i,j)*FI(j,1)
                    end do
                    res = res+r**2
                    FI(i,1) = FI(i,1) + r/LapPbigD(i,i)
                end do
            iter = iter + 1
        end do
    
    pbvecbig_M = FI - matmul(matmul(MI,transpose(EU)),FI)
    p = 0.0d0
     do j = 0,ny
        do i = 0,nx
           p(i,j) = pbvecbig_M(1+i+j*(nx+1),1)
        end do
     end do
        ...
        do while (( res/norm .gt. 1e-4) &
               .and. (iter .lt. 1e12))
                ...
                do i = 1,sys_size_pbig-1
                    r = EU(i,ik)
                    do j = 1,sys_size_pbig-1
                        r = r - LapPbigD(i,j)*MI_temp(j)
                    end do
                    res = res+r**2
                    MI(i,ik) = MI_temp(i) + r/LapPbigD(i,i)
                end do
            iter = iter + 1
        end do
    end do
        ...
        do while (( res/norm .gt. 1e-4) &
               .and. (iter .lt. 1e12))
                ...
                do i = 1,sys_size_pbig-1
                    r = pbvecbig(i)
                    do j = 1,sys_size_pbig-1
                        r = r - LapPbigD(i,j)*FI_temp(j)
                    end do
                    res = res+r**2
                    FI(i,1) = FI_temp(i) + r/LapPbigD(i,i)
                end do
            iter = iter + 1
        end do
  ...
  if (use_dense_p) then
    ...
    pold = 0.0d0
     do j = 0,ny
        do i = 0,nx
           pold(i,j) = pbvecbig_M(1+i+j*(nx+1),1)
        end do
     end do
    elseif (method == "Jacobi") then
    do ik = 1,3
        ...
        do while ((res/norm .gt. 1e-6_dp) &
             .and. (iter .lt. 1e12))
              ...
              do i = 1,sys_size_uv
                  r = uvec(i)
                  do j = 1,sys_size_uv
                      r = r - Lapuv(i,j)*uvec_temp(j)
                  end do
                  uvec_k(i) = uvec_temp(i) + r/Lapuv(i,i)
                  res = res + r**2
              end do
              iter = iter + 1
        end do
        ...
        do while ((res/norm .gt. 1e-6_dp) &
             .and. (iter .lt. 1e12))
              ...
              do i = 1,sys_size_uv
                  r = vvec(i)
                  do j = 1,sys_size_uv
                      r = r - Lapuv(i,j)*vvec_temp(j)
                  end do
                  vvec_k(i) = vvec_temp(i) + r/Lapuv(i,i)
                  res = res + r**2
              end do
              iter = iter + 1
        end do
