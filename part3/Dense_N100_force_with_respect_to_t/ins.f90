...
program ins
  ...
  ! hx = Lx/Nx, hy = Ly/Ny.
  real(dp) :: hx,hy,time1,time2,r,res,norm,tc
...
,uvec_temp((Nx-1)*(Ny-1)),vvec_temp((Nx-1)*(Ny-1)))
  ...
  call setupRhsideP(pbvecbig,nx,ny,hx,hy,u,v,alpha,pbx,pby,tc)
    ...
    do while ((res/norm .gt. hx**2) &
         .and. (iter .lt. 10000))
          ...
          do i = 1,sys_size_pbig-1
              r = pbvecbig(i)
              do j = 1,sys_size_pbig-1
                  r = r - LapPbig(i,j)*pbvecbig_temp(j)
              end do
              pbvecbig_k(i) = pbvecbig_temp(i) + r/LapPbig(i,i)
              res = res + r**2
          end do
          iter = iter + 1
    end do
  ...
  call setupRhsideP(pbvecbig,nx,ny,hx,hy,uold,vold,alpha,pbx,pby,tc)
  ...
  do nt = 1,Nsteps
     ...
     call setupRhsideUV(Uvec,Vvec,nx,ny,hx,hy,gux,guy,gvx,gvy,&
          Leu,Lev,Leuold,Levold,Liu,Liv,u,v,k,nu,tc)
     ! Swap
     uold = u
     vold = v
        ...
        do while ((res/norm .gt. 1e-12_dp) &
             .and. (iter .lt. 10000))
              ...
              do i = 1,sys_size_uv
                  ...
                  uvec_k(i) = vvec_temp(i) + r/Lapuv(i,i)
                  res = res + r**2
              end do
              iter = iter + 1
        end do
...
subroutine setupRhsideP(bP,nx,ny,hx,hy,u,v,alpha,pbx,pby,tc)
  use type_defs
  implicit none
  integer, intent(in)     :: nx,ny
  real(dp), intent(in)    :: hx,hy,alpha,tc
  real(dp), intent(in)    :: u(-1:nx+1,-1:ny+1),v(-1:nx+1,-1:ny+1)
  ...
  do j = 1,ny-1
     do i = 1,nx-1
        k = 1+i + j*(nx+1)
        bP(k) = F(i,j)+  exp(-tc)*100*(hx*i - 3*(hx*i)**2 + 2*(hx*i)**3 + (hy*j) - 3*(hy*j)**2 + 2*(hy*j)**3)
     end do
  end do
...
subroutine setupRhsideUV(bU,bV,nx,ny,hx,hy,gux,guy,gvx,gvy,Leu,Lev,Leuold,Levold,&
  ...
  implicit none
  integer, intent(in)   :: nx,ny
  real(dp), intent(in)  :: hx,hy,nu,k,tc
  ...
  do j = 1,ny-1
     do i = 1,nx-1
        l = i + (j-1)*(nx-1)
        bU(l) = bU(l) + FFu(i,j)+ exp(-tc)*100*(hx*i)**2*(1 - hx*i)**2
        bV(l) = bV(l) + FFv(i,j)+ exp(-tc)*100*(hy*i)**2*(1 - hy*i)**2
     end do
  end do
